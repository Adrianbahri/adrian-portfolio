    ...
    <section id="work" className="w-full py-24 bg-transparent">
      <div className="section-container">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          ...
          <div className="flex p-1 bg-surface-100 border border-border-strong rounded-[5px] relative shadow-2xl">
            <button 
              ...
              className={cn(
                "relative z-10 px-8 py-2 text-[10px] font-heading font-bold uppercase tracking-[0.2em] transition-colors duration-300 rounded-[3px]",
          ...
          <AnimatePresence mode="popLayout">
            {displayProjects.map((project, i) => (
              <motion.div
                ...
                className={cn(
                  "relative block bg-transparent border border-white/5 transition-all duration-500 hover:border-primary/20",
              ...
              >
                ...
                >
                  <div className="relative aspect-[16/10] overflow-hidden bg-canvas border border-white/5">
                    <img 
                      src={project.img} 
                      alt={project.title}
                    ...
                    <div className="absolute top-3 right-3">
                      <span className="px-3 py-1 bg-surface-200/90 backdrop-blur-md text-[0.55rem] font-bold tracking-[0.2em] text-on-dark uppercase border border-border-strong">
                        {project.category}
                      </span>
                    </div>
                    ...
                    <div className="absolute inset-0 bg-canvas/60 opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-center backdrop-blur-[2px]">
                      <span className="px-6 py-2 border border-primary text-primary text-[0.6rem] font-bold uppercase tracking-[0.2em] flex items-center gap-2">
                  ...
                  <div className="mt-6 space-y-2">
                    ...
                    <p className="text-xs text-body-muted leading-relaxed font-sans line-clamp-2">
                      {project.desc}
                    </p>
                    {mode === 'developer' && project.tags && (
                      <div className="flex flex-wrap gap-2 pt-2">
                        {project.tags.slice(0, 3).map((tag: string, i: number) => (
                          <span key={i} className="text-[7px] text-on-dark/30 font-bold uppercase tracking-widest border border-white/5 px-2 py-0.5">
                            {tag}
                          </span>
                        ))}
                      </div>
        ...
        >
          <Link 
            href="/projects"
            className="group flex items-center gap-3 px-10 py-4 border border-border-strong text-on-dark font-heading text-[0.65rem] font-bold uppercase tracking-[0.2em] hover:bg-on-dark hover:text-canvas transition-all duration-500 rounded-[5px]"
          >
            View All Projects
