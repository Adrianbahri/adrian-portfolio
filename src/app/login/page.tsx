...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // No need to set loading to false here, the navigation will happen
    }
  };
      ...
      >
        <div className="bg-[#0a0a0a] border border-white/5 rounded-[3rem] p-12 shadow-2xl backdrop-blur-xl">
          <header className="mb-12 text-center">
            ...
            <h1 className="text-3xl font-heading font-medium italic mb-2">Adrian <span className="text-primary">Terminal</span></h1>
            <p className="text-white/40 text-sm uppercase tracking-[0.2em] font-bold">Authorized Access Only</p>
          </header>

          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-2">
              <label className="text-[0.65rem] font-bold uppercase tracking-widest text-white/40 ml-4">Identifier</label>
              <div className="relative group">
                <input 
                  ...
                  className="w-full bg-white/[0.03] border border-white/5 rounded-2xl px-6 py-4 text-sm focus:border-primary/50 outline-none transition-all"
                  required
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-[0.65rem] font-bold uppercase tracking-widest text-white/40 ml-4">Access Key</label>
              <div className="relative group">
                <input 
                  type="password" 
                  value={password}
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="w-full bg-primary text-black py-5 rounded-2xl font-bold uppercase tracking-[0.2em] text-sm hover:bg-primary/90 transition-all flex items-center justify-center gap-3 shadow-[0_20px_40px_rgba(var(--primary-rgb),0.2)]"
            >
        ...
        >
          Return to public site <ArrowRight size={12} />
        </button>
      </motion.div>
    </main>
  );
