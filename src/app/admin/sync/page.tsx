...
export default function AdminSync() {
  ...
  const syncAll = async () => {
    ...
    try {
      const projectsToSync = staticProjects.map(p => ({
        title: p.title,
        slug: p.slug,
        category: p.category,
    ...
    try {
      const articlesToSync = staticArticles.map(a => ({
        title: a.title,
        slug: a.slug,
        category: a.category,
    ...
    try {
      const expToSync = workData.map(w => ({
        title: w.role,
        company: w.company,
        timeline: w.year,
        location: w.location,
    ...
    try {
      const orgsToSync = organizationData.map(o => ({
        role: o.role,
        company: o.company,
        year: o.year,
        desc: o.desc
    ...
    try {
      const volToSync = volunteerData.map(v => ({
        role: v.role,
        company: v.company,
        year: v.year,
        desc: v.desc
    ...
    try {
      const achToSync = achievementsData.map(a => ({
        title: a.title,
        event: a.event,
        year: a.year,
        desc: a.desc,
